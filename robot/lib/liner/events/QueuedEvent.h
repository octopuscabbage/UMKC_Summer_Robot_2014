...
class QueuedEvent : public SensedEvent{
	public:
		...
		virtual bool isNecessary(){
			...
			Serial.println(((testRequirement())? "required" : "not required"));
	...
	private:
		inline bool testRequirement(){
			return ((middleSensor && leftSensor) || (middleSensor && rightSensor) );
		}
		...
		void turnStr(String& currentString){
